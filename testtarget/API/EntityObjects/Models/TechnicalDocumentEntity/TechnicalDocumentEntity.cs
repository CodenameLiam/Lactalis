...
{
	public class TechnicalDocumentEntity : BaseEntity, IFileContainingEntity 
	{
		...
		/// </summary>
		/// <see cref="Lactalis.Models.TechnicalDocumentCategory"/>
		public Guid? TechnicalDocumentCategoryId { get; set; }


		public TechnicalDocumentEntity()
		{
			EntityName = "TechnicalDocumentEntity";

			InitialiseAttributes();
			InitialiseReferences();
		}

		public TechnicalDocumentEntity(ConfigureOptions option)
		{
			Configure(option);
			InitialiseAttributes();
			InitialiseReferences();
		...
		private void InitialiseReferences()
		{
			...
			{
				EntityName = "TechnicalDocumentCategoryEntity",
				OppositeName = "TechnicalDocumentCategory",
		...
		private static string GetInvalidTechnicalDocumentCategoryId(string validator)
		{
			switch (validator)
			{
				case "Required":
					return "";
				default:
					throw new Exception($"Cannot find validator {validator} for attribute Technical Documents");
			}
		}
		...
		public override Dictionary<string, string> ToDictionary()
		{
			...
			if (TechnicalDocumentCategoryId != default)
			{
				entityVar["technicalDocumentCategoryId"] = TechnicalDocumentCategoryId.ToString();
			}
		...
		public override void SetReferences (Dictionary<string, ICollection<Guid>> entityReferences)
		{
			foreach (var (key, guidCollection) in entityReferences)
			{
				switch (key)
				{
					case "TechnicalDocumentCategoryId":
						ReferenceIdDictionary.Add("TechnicalDocumentCategoryId", guidCollection.FirstOrDefault());
		...
		private void SetOneReference (string key, Guid guid)
		{
			...
			{
				case "TechnicalDocumentCategoryId":
					TechnicalDocumentCategoryId = guid;
					break;
				default:
					throw new Exception($"{key} not valid reference key");
			}
		...
		// TODO needs some warning if trying to get an invalid entity, and the entity
		// attributes don't actually have any validators to violate.
		public static TechnicalDocumentEntity GetEntity(bool isValid, string fixedValue = null)
		{
		...
		public static TechnicalDocumentEntity GetInvalidEntity()
		{
			var technicalDocumentEntity = new TechnicalDocumentEntity
			{
			};
		...
		/// Gets an entity with attributes that conform to any attribute validators.
		/// </summary>
		public static TechnicalDocumentEntity GetValidEntity(string fixedStrValue = null)
		{
			var technicalDocumentEntity = new TechnicalDocumentEntity
			{
			...
			technicalDocumentEntity.FileId = technicalDocumentEntity.File.Id;
