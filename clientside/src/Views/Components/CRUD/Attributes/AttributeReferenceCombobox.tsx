...
class AttributeReferenceCombobox<T extends Model> extends React.Component<
	...
	private onChange = (event: React.SyntheticEvent<HTMLElement>, data: DropdownProps) => {
		...
		if (value !== undefined && value !== null) {
			if (this.props.fetchReferenceEntity) {
				this.props.model[this.props.options.attributeName] = _.find(
	...
	private fetchOptions = async (query?: string | string[]) => {
		...
		return store.apolloClient
			.query({
				...
				variables: {
					args: query
						? new this.props.referenceType().getSearchConditions(
								this.props.fetchReferenceEntity ? query["id"] : query
						  )
						: null,
					take: 10,
				},
			...
			.then(({ data }) => {
				...
				if (this.props.model.id !== null && this.props.model.id !== undefined) {
					associatedObjects = associatedObjects.filter(
						(d: IModelAttributes) => d.id !== this.props.model.id
					);
				}
				...
				if (associatedObjects) {
					...
					comboOptions = optionObjects.map((obj) => ({
