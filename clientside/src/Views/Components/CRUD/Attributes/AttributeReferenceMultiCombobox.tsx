 ...
 */
@observer
export default class AttributeReferenceMultiCombobox<T extends Model> extends React.Component<
	IAttributeReferenceComboboxProps<T>
> {
	...
	private get joinProp() {
		return this.props.options.attributeName.substring(
			0,
			this.props.options.attributeName.length - 1
		);
	}
	...
	public get resolveFunc() {
		...
		return () =>
			store.apolloClient.query({ query: query, fetchPolicy: "network-only" }).then((data) => {
				...
				if (associatedObjects) {
					comboOptions = associatedObjects
						.map((obj) => new this.props.referenceType(obj))
	...
	public mutateOptions = (query: string | string[]): Promise<ComboboxOption<Model>[]> => {
		...
		return this.resolveFunc(query).then((e) => {
			...
			return e.map((x) => {
				const option = {
					display: isJoinEntity
						? x.value[this.joinProp].getDisplayName()
						: x.value.getDisplayName(),
					value: x.value,
					isFixed: false,
				};
			...
			<MultiCombobox<T, Model>
				...
				getOptionValue={(option) => {
					return this.props.isJoinEntity
						? option
							? option[this.props.options.attributeName.slice(0, -1)].id
							: undefined
						: option
						? option.id
						: undefined;
