...
import { FarmerEntity, AdminEntity } from "Models/Entities";
...
export default class UserList extends React.Component<RouteComponentProps<IEntityEditRouteParams>> {
	...
	private get searchParams() {
		...
		if (this.search.searchTerm.match(regex)) {
			return [
				params.concat({ comparison: "Equal", path: "id", value: [`${this.search.searchTerm}`] }),
			];
		}
		return [params];
	}
	...
	private fetchData = () => {
		axios
			.post(`${SERVER_URL}/api/account/users`, {
	...
	private setData = (data: any) => {
		this.totalRecords = data.data.totalUsers;
		...
		if (userTypes[entity.discriminator] !== undefined) {
				...
				tableActions.push({
					action: (entity: IUser) => {
						this.props.history.push({
							pathname: `/admin/${entity.discriminator}/view/${entity.id}`,
						});
					},
					label: "View",
					showIcon: true,
			...
			if (SecurityService.canUpdate(userTypes[entity.discriminator])) {
				tableActions.push({
					action: (entity: IUser) => {
						this.props.history.push({
							pathname: `/admin/${entity.discriminator}/edit/${entity.id}`,
						});
					},
					label: "Edit",
					showIcon: true,
				...
				tableActions.push({
					action: (entity: IUser) => {
						confirmModal("Please confirm", "Are you sure you want to delete this item?").then(
							() => {
								new userTypes[entity.discriminator](entity)
									.delete()
									.then(() => {
										this.fetchData();
		...
		return (
			<Modal
				...
				<div key="actions" className="modal__actions">
					<Button key="confirm" onClick={this.userSelected} display={Display.Solid}>
						Confirm
					</Button>
					<Button key="cancel" onClick={this.onCancelModal} display={Display.Outline}>
						Cancel
					</Button>
				</div>
			</Modal>
			...
			<>
				...
				<Collection
					...
					pagination={{
						...
						queryOptions: this.paginationQueryOptions,
					}}
		...
		if (user === undefined) {
			...
			return (
				<Modal
					...
					<p>
						Are you sure you wish to {this.activateModalState.activate ? "activate" : "deactivate"}{" "}
						{user.email}?
					</p>
					<div key="actions" className="modal__actions">
						<Button
							key="confirm"
							onClick={() =>
								this.activateModalState.activate
