...
describe("Collection Component", () => {
	...
	it(`Should have the correct items displayed on the first page`, () => {
		...
		expect(component.find(".collection__item")).toHaveLength(
			paginationSettings.queryOptions.perPage
		);
	});
	...
	it("Should end on the last page", () => {
		...
		component.setState({ forceUpdateCall: 1 });
		expect(component.find("span.pagination__page-number").text()).toEqual(
			`${totalPages} of ${totalPages}`
		);
		expect(component.find(".collection__item")).toHaveLength(
			data.length % paginationSettings.queryOptions.perPage
		);
	});
