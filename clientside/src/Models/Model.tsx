...
export function entity(modelName: string, displayName?: string) {
	return (target: object) => {
		...
		target[displayNameSymbol] = displayName === undefined ? modelName : displayName;
	};
}
...
export class Model implements IModelAttributes {
		...
		return Object.keys(this.validationErrors).some((prop) => {
			...
			switch (propErrors.type) {
				...
				case PropertyType.CHILDREN:
					return (this.validationErrors[prop].errors as Array<IEntityValidationErrors>).some(
						(childErrors) => {
							return Object.keys(this.validationErrors[prop].errors).length > 0;
						}
					);
	...
	public getErrorsForAttribute = (attributeName: string): string[] => {
		if (!!this.validationErrors[attributeName]) {
			if (this.validationErrors[attributeName].type === PropertyType.OWN) {
				return _.chain(Object.keys(this.validationErrors[attributeName].errors))
					.map((error) => {
		...
		var promise = new Promise<IEntityValidationErrors>((resolve, reject) => {
			...
			co(function* () {
				...
				if (that[validatorSymbol]) {
					const ownErrorsArrray: IModelAttributeValidationError[] = (yield Promise.all(
						(that[validatorSymbol] as IModelValidator[]).map((validator) => validator(that))
					)).filter((error: IModelAttributeValidationError) => {
						if (!_.some(path, _.isEmpty)) {
							return !!error;
						} else {
							return !!error &&
								!!error.attributeName &&
								(path as object).hasOwnProperty(error.attributeName)
								? !!error
								: null;
						}
					});
				...
				const referencesToValidate = Object.keys(path).filter((key) =>
					that.references.includes(key)
				);
				...
				for (const reference of referencesToValidate) {
					if (Array.isArray(that[reference])) {
						const childrenErrors: IEntityValidationErrors[] = (yield Promise.all(
							that[reference].map((item: Model) => item.validate(path[reference]))
						...
						const referenceErrors = (yield that[reference].validate(
							path[reference]
						)) as IEntityValidationErrors;
	...
	public getAttributeCRUDOptions(): AttributeCRUDOptions[] {
		...
		for (const [attributeName, displayOptions] of Object.entries(this[crudOptions])) {
			attributeDisplay.push(
				new AttributeCRUDOptions(attributeName, displayOptions as ICRUDOptions)
			);
		}
	...
	public async deleteWhere(
		conditions?: Array<IWhereCondition<Model>> | Array<Array<IWhereCondition<Model>>>,
		ids?: string[]
	) {
	...
	public async save(relationPath: {} = {}, options: ISaveOptions = {}) {
		...
		const mutation = gql`
			mutation ${functionName}($${modelsName}:${graphQlInputType}${variables
			.map((v) => `,$${v.key}:${v.graphQlType}`)
			.join(",")}) {
	 ...
	 */
	public getSearchConditions<T extends Model>(
		searchTerm?: string
	): Array<Array<IWhereCondition<T>>> | undefined {
		if (searchTerm && searchTerm.trim() !== "") {
			const crudOptions = this.getAttributeCRUDOptions();
			const values: IWhereCondition<T>[] = _.chain(crudOptions)
				...
				.flatMap((attributeOption) => {
					...
					if (valid) {
						return [
							{ option: attributeOption, search: valid.query, extraOptions: valid.extraOptions },
						];
					}
					return [];
				})
	 ...
	 */
	public getFilterConditions<T>(
		filterConfig: ICollectionFilterPanelProps<T>
	): IWhereCondition<T>[][] | undefined {
		if (
			filterConfig &&
			filterConfig.filters &&
			filterConfig.filters.some((filter) => filter.active === true)
		) {
			let filterConditions = _.chain(filterConfig.filters)
						...
						if (filter.displayType === "datepicker") {
							return [
								...
								},
								{
									...
									value1: moment(filter.value2 as Date)
										.add("day", 1)
										.format("YYYY-MM-DD"),
								},
							];
